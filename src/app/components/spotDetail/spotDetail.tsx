...
const SpotDetail = ({ dragon }: SpotDetailProps) => {
    ...
    return (<motion.div
        ...
        <Link title={dragon.name} href={"/dragon/" + dragon.id} className={`${style.image} `}>
        ...
        <div className={style.infos}>
            <div>
                <h2 id="dragonName" data-testid="dragonName" className={`${style.title}`}>
            ...
            <span data-testid="dragonType" id="dragonType" className={`${style.type} type-${dragon.type.toLowerCase().trim()}`}>
                {dragon.type}
            </span>
        </div>
    </motion.div>)
